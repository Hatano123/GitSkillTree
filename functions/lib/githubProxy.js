...
function hasOnlyQuery(url, expected) {
...
function isSafeEncodedPath(value) {
    try {
...
export function parseGithubApiPath(value) {
    ...
    if (url.origin !== GITHUB_API_ORIGIN || url.hash)
        return null;
    ...
    if (contentsMatch && isSafeEncodedPath(contentsMatch[3]) && url.searchParams.size === 1) {
        ...
        if (ref && ref.length <= 255 && isSafeEncodedPath(ref))
            return `${url.pathname}?${url.searchParams.toString()}`;
    }
    return null;
}
...
async function getInstallationToken(appId, installationId, privateKey) {
    ...
    if (!response.ok)
        throw new Error(`GitHub installation authentication failed with HTTP ${response.status}.`);
    const data = await response.json();
    if (!data.token || !data.expires_at)
        throw new Error('GitHub installation authentication returned an invalid response.');
    cachedInstallationToken = { token: data.token, expiresAt: new Date(data.expires_at).getTime() };
    return data.token;
}
export async function fetchGithubApi(path, appId, installationId, privateKey) {
    ...
    if (Buffer.byteLength(text, 'utf8') > MAX_RESPONSE_BYTES)
        throw new Error('GitHub response exceeded the proxy size limit.');
    let body;
    try {
...
//# sourceMappingURL=githubProxy.js.map